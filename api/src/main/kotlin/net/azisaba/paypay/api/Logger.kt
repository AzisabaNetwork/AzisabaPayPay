...
import net.azisaba.paypay.api.AzisabaPayPayAPIProvider.Companion.getAPI
import net.azisaba.paypay.api.util.ReflectionUtil
...
interface Logger {
    ...
    companion object {
        fun createByProxy(instance: Any): Logger {
            ...
            return Proxy.newProxyInstance(Logger::class.java.getClassLoader(), arrayOf<Class<*>>(Logger::class.java)) { _, method, args ->
                val m = ReflectionUtil.findMethod(instClass, method)
                    ?: throw RuntimeException(instClass.getTypeName() + " does not implements " + method.toGenericString())
            ...
            return object : Logger {
                private fun format(msg: String): String {
                    ...
                    while (formatted.contains("{}")) {
                        formatted = formatted.replaceFirst("\\{}".toRegex(), "{" + length++ + "}")
                    }
                    return formatted
                }
        ...
        val DEFAULT_LOGGER = createFromJavaLogger(java.util.logging.Logger.getLogger("InterChat"))
    }
}
